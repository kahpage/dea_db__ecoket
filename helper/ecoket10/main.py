...
def main():
    ...
    raw_url = "https://web.archive.org/web/20070222041448id_/http://eco.ket.jp:80/ecoket10_clist.htm"
    ...
    for table in tables:
        ...
        for row in table_rows:  # Skip header row
                ...
                circle_name_furigana = sanitize_string(cols[1].get_text())
                ...
                if circle_url_tag and circle_url_tag.has_attr("href"):
                    circle_urls.append(circle_url_tag["href"].replace("https://web.archive.org/web/20070222041448/", ""))
                position = sanitize_string(cols[3].get_text())

                comment_parts = []
                if circle_name_furigana:
                    comment_parts.append(f"Name furigana: {circle_name_furigana}")
