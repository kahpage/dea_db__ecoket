...
def main():
    ...
    raw_url = "https://web.archive.org/web/20011231002259id_/http://sakura.comike.to/ecoket/c_list3.htm"
    ...
    for table in tables:
        ...
        for row in table_rows:  # Skip header row
                ...
                if len(cols) < 3:
                    ...
                    continue
                if len(cols) > 4:
                    print("Skipping row with too many columns:", row)
                    continue
                if len(cols) == 4:
                    # new block
                    current_block = sanitize_string(cols[0].get_text())
                    cols = cols[1:] # Shift columns to the left
                
                position_number = sanitize_string(cols[0].get_text())
                ...
                if circle_url_tag and circle_url_tag.has_attr("href"):
                    circle_urls.append(circle_url_tag["href"].replace("https://web.archive.org/web/20011231002259/", ""))

                circle = Circle(
                    aliases=[circle_name],
                    # pen_names=[pen_name] if pen_name else None,
                    links=circle_urls if circle_urls else None,
                    position=f"{current_block} {position_number}".strip() if current_block or position_number else None,
