...
if __name__ == '__main__':
    ...
    for file in PATH_CURRENT.glob("*.htm"):
        ...
        for row in rows:
            ...
            if len(cols) == 4:
                start_index = 1
                current_letter = cols[0].get_text(strip=True)

            number = cols[start_index].get_text(strip=True)
            position = f"{current_letter}{number}"
            name = cols[start_index+1].get_text(strip=True)
            if not is_to_add(name) or "サークル名" in name: 
                continue

            links = []
            web_tag = cols[start_index+2].find("a")
